function grid(sp); 
    ...
    buffer=50;  % size in pixels of region surrounding the '+' fixation
    cellx=70;    % x-side in pixels
    celly=70;    % y-side in pixels
    gridnmb=3;   % cell units per side
    
    gridAx=Xcentre-(buffer/2);  % A=lower right corner of space9
    gridAy=Ycentre-(buffer/2);  
    gridBx=Xcentre+(buffer/2);  % B=lower left corner of space16
    gridBy=Ycentre-(buffer/2);
    gridCx=Xcentre-(buffer/2);  % C=upper right corner of space21
    gridCy=Ycentre+(buffer/2);
    gridDx=Xcentre+(buffer/2);  % D=upper left corner of space28
    gridDy=Ycentre+(buffer/2);
