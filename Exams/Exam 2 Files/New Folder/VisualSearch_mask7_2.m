%   %VisualSearch.m
%  7.2 cover images with a mask
% Change 1: offtime is now 800ms (line 92)
% Change 2: make the maks texture (line 159)  
% Change 3: add in the call to the mask function to make the mask
           %texture for the distractors (line 216)
% Change 4: Add in the call to the mask function to make the mask
           %texture for the targets and onevery possible stimulus location (line 228-238)
%Change 5: show the mask for 100ms using a while loop:(lines
            %295-303)
  %-------------------------------------------------------------
%   Interrupted Visual Search Experiment. line 12
...
%   Last Updated November 30, 2005 line 21
%-------------------------------------------------------------
...
%d) change hte trial number (number of trials called; T_TRIALS) to a lower
...
HideCursor;    
    ...
    for itrial=1:T_TRIALS
        ...
        % search Screen: draw all of the targets and distractors onto the
        % 'search' screen. 
     ...
     %then put the colors into a variable called colour, the size of the setsize -1.    
        ...
        for i=1:36
            if loc(i)==1   %1 is for distractors
                ...
                ell(search,tempcol,tempor,tempx,tempy); %place an L on the 'search' grid, using the colour, orientation, 
                %and x and y coordinates (of the top left corner) just defined.
           %change 3: add in the call to the mask function to make the mask
           %texture for the distractors (line 217)
                mask(masker, tempx, tempy) 
                n=n+1;
