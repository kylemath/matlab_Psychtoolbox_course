...
%     warning off MATLAB:DeprecatedLogicalAPI	  
    ...
    [window,rect]=Screen('OpenWindow',max(Screen('Screens')),background, [0 0 640 480]);
    ...
    %---------------------------------------
    % Change the offtime to 800ms
    %---------------------------------------
    offtime=refresh_rate-2*ontime; %900 ms
...
%--------------------------------------------    
    ...
    for itrial=1:T_TRIALS
        ...
        %---------------------------------------
        % Half of index of loc will be assigned
        % to 0, and drawing masks based on this
        % set of 1 and 0.
        %---------------------------------------
        idx = ones(1,numel(loc));
        half = round(length(idx)/2);
        a = 1:length(idx);
        for i = 1:half
            j = randi(length(a));
            a(j) = [];
        ...
        for i=1:36
            if loc(i)==1   %1 is for distractors
                ...
                %-----------------------------------------------------
                % mask the target based on current x and y
                %-----------------------------------------------------
                if idx(i) == 1
                    mask(search, tempx, tempy);
                end
                n=n+1;
            ...
            %-----------------------------------------------------
            % To display masks even neither 'L' nor 'T' were drawn
            %-----------------------------------------------------
            else
                grid(i);
                tempor=fix(rand(1)*4)+1;
