...
%--------------------------------------------    
    ...
    DATA=repmat(struct('setsize',1,'tid',1,'maskstate',1,'rt',-1,'resp',-1,'error',-1),... % Record which mask state (present or not) you are using at the moment 
        1,T_TRIALS);
      % setsize 16, 32
    ...
    for i=1:T_TRIALS
        ...
        DUMMY(i).maskstate=mod(i,2)+1; % Randomize whatever is in the data so just choose between the two values of every trial that it goes through
    end;
...
%--------------------------------------------    
    ...
    for itrial=1:T_TRIALS
        ...
        locmask = Screen('MakeTexture',window,white_background);  %Create the location of the mask on the screen 
        ...
        % Then put the "window pane" mask
        % Add a third condition for the part of the grid that has no
        ...
        for i=1:36
            ...
            elseif loc(i)==2 %2 is for target
                ...
                mask(locmask, tempx, tempy);
            elseif loc(i) == 0  %0 is for empty spaces
                grid(i);
                tempor=fix(rand(1)*4)+1;
                tempx=cx+(round(rand(1)*jitter))+ offset;
                tempy=cy+(round(rand(1)*jitter))+ offset;
  ...
  %%%%%%% start search task
	  ...
	  while ~KbCheck & (s < totaltime),
            ...
            c=0;
            while ~KbCheck & (c < ontime), % On time is when the mask comes on the screen
                if DATA(itrial).maskstate == 1 % Select the mask on if it is 1
                  Screen('DrawTexture',window,locmask,[],[]);
                  Screen('Flip',window);
                c=c+1;
                s=s+1;
                elseif DATA(itrial).maskstate == 2 % Do not show the mask
                    c=c+1;
                    s=s+1;
                end
            end;
...
%--------------------------------------------  
	...
	fid=fopen(['Visual_Search' num2str(SbjID) '.out'],'a');
   	fprintf(fid, 'Subject     Trial   ss  tid   maskstate    RT   resp  Error\r');
   
	for i=1:endtrial
		fprintf(fid, '%-8s   %3d   %2d    %d   %d   %4.0f      %d     %d\r', ...
                     num2str(SbjID), DATA(i).trial, DATA(i).setsize, ...
                     DATA(i).tid, DATA(i).rt, DATA(i).maskstate, DATA(i).resp, DATA(i).error);
    end;
    fclose(fid);
    clear all;
